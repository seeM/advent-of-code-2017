...
def string_to_list(string):
    """
    Convert the string spreadsheet to a list.
    """
    arr = []
...
def checksum(string):
    """
    Compute the checksum of a spreadsheet; the sum of ranges of rows.
    """
    arr = string_to_list(string)
    ranges = [range_(x) for x in arr]
    return sum(ranges)

def divisibles(arr):
	"""
	Return the ratio of two divisible integers in a list.
	"""
	for i in range(len(arr) - 1):
		for j in range(i + 1, len(arr)):
			y, x = sorted([arr[i], arr[j]])
			if x % y == 0:
				return x // y

def checksum_divisible(string):
	"""
	Compute the sum of ratios of divisible integers in each row.
	"""
	arr = string_to_list(string)
	divis = [divisibles(x) for x in arr]
	return sum(divis)
...
if __name__ == '__main__':
    ...
    print(checksum_divisible(INPUT))
